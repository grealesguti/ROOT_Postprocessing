...
def filter_values(x, y, range_val, axis=0):
    ...
    if axis == 0:  # Filtering along y
        print('filtering in y')
        y0 = min(y)
        y1 = y0 + range_val
        ymax = max(y)
    ...
    elif axis == 1:  # Filtering along x
        print('filtering in x')
        x0 = min(x)
        x1 = x0 + range_val
        xmax = max(x)
        # Iterate through the x and y values
        while x0 < xmax:
            for i in range(len(x)):
                if x0 <= x[i] <= x1:
                    if not filtered_y or y[i] <= min(filtered_y):
                        filtered_x.append(x[i])
                        filtered_y.append(y[i])
                        filtered_indexes.append(i)
            x0 = x1
